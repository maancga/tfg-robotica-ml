...
from openai_ros.robot_envs import pioneer3dx_env
...
class Pioneer3dx(pioneer3dx_env.Pioneer3dx):
    def __init__(self):
        # This is the path where the simulation files, the Task and the Robot gits will be downloaded if not there
        # This parameter HAS to be set up in the MAIN launch of the AI RL script
        ros_ws_abspath = rospy.get_param("/pioneer3dx/ros_ws_abspath", None)
        ...
        LoadYamlFileParamsTest(rospackage_name="openai_ros",
                               rel_path_from_package_to_file="src/openai_ros/task_envs/pioneer3dx/config",
                               yaml_file_name="pioneer3dx.yaml")

        # Here we will add any init functions prior to starting the MyRobotEnv
        super(Pioneer3dx, self).__init__(ros_ws_abspath)
        # Only variable needed to be set here
        number_actions = rospy.get_param('/pioneer3dx/n_actions')
        ...
        self.dec_obs = rospy.get_param("/pioneer3dx/number_decimals_precision_obs", 1)
        self.linear_forward_speed = rospy.get_param('/pioneer3dx/linear_forward_speed')
        self.linear_turn_speed = rospy.get_param('/pioneer3dx/linear_turn_speed')
        self.angular_speed = rospy.get_param('/pioneer3dx/angular_speed')
        self.init_linear_forward_speed = rospy.get_param('/pioneer3dx/init_linear_forward_speed')
        self.init_linear_turn_speed = rospy.get_param('/pioneer3dx/init_linear_turn_speed')
        
        
        self.n_observations = rospy.get_param('/pioneer3dx/n_observations')
        self.min_range = rospy.get_param('/pioneer3dx/min_range')
        self.max_laser_value = rospy.get_param('/pioneer3dx/max_laser_value')
        self.min_laser_value = rospy.get_param('/pioneer3dx/min_laser_value')
        ...
        self.laser_filtered_pub = rospy.Publisher('/pioneer3dx/laser/scan_filtered', LaserScan, queue_size=1)
    ...
    def _is_done(self, observations):
        ...
        if self._episode_done:
            rospy.logdebug("Pionner3dx is Too Close to wall==>"+str(self._episode_done))
        else:
            rospy.logerr("Pionner3dx is Ok ==>")
